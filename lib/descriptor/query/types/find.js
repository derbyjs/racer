...
exports.onOverwriteNs = function (docs, findQuery, model) {
  ...
  for (var i = 0, l = docs.length; i < l; i++) {
    if (! docs[i]) {
      console.warn(new Error('Unexpected: docs[i] is undefined'));
      console.warn('docs:', docs, 'i:', i);
    } else {
...
function insertDocAsPointer (comparator, model, pointerPath, currResults, doc) {
  ...
  for (var k = currResults.length; k--; ) {
    ...
    if (comparison >= 0) {
      if (! currRes) {
        console.warn(new Error('Unexpected: expected currResults[k] !== undefined'));
        console.warn('currResults:', currResults, 'k:', k);
        continue;
      }
