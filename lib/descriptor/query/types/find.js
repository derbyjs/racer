...
exports.onUpdateDocProperty = function (doc, memoryQuery, model, searchSpace, currResult) {
  var id = doc.id
    ...
    , comparator = memoryQuery._comparator;
  ...
  if (memoryQuery.filterTest(doc, ns)) {
    ...
    if (~pos) {
      ...
      var resortedResults = comparator ? currResult.sort(comparator) : currResult
    ...
    return insertDocAsPointer(comparator, model, pointerPath, currResult, doc);
  }
