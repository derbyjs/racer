...
var finishAfter = require('./async').finishAfter;
...
Promise.prototype.resolve = function(err, value) {
  if (this.resolved) {
...
Promise.prototype.on = function(callback) {
  if (this.resolved) {
...
Promise.parallel = function(promises) {
  ...
  promises.forEach(function(promise, i) {
    promise.on(function(err, val) {
      if (composite.resolved) return;
      if (err) return composite.resolve(err);
