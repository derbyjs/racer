...
Model.prototype._initQueries = function(items, contexts) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    queryMap[query.hash] = query;
  }
  for (var contextId in contexts) {
    var context = contexts[contextId];
    var contextModel = this.context(contextId);
    for (var hash in context.fetchedQueries) {
      var count = context.fetchedQueries[hash];
      var query = queryMap[hash];
      while (count--) contextModel._context.fetchQuery(query);
    }
    for (var hash in context.subscribedQueries) {
      var count = context.subscribedQueries[hash];
      var query = queryMap[hash];
      // TODO: This is a gross hack, and it will probably be wrong if two
      // queries with the same hash are subscribed in different contexts
      query.model = contextModel;
