...
Model.prototype.query = function(collectionName, expression, options) {
  ...
  return this._getOrCreateQuery(collectionName, expression, options);
};

/**
 * If an existing query is present with the same context, `collectionName`,
 * `expression`, and `options`, then returns the existing query
 *
 * @param {string} collectionName
 * @param {*} expression
 * @param {*} options
 */
Model.prototype._getOrCreateQuery = function(collectionName, expression, options) {
...
Model.prototype._initQueries = function(items) {
  for (var i = 0; i < items.length; i++) {
    ...
    var query = model._getOrCreateQuery(collectionName, expression, options);
