...
Model.prototype.query = function(collectionName, expression, source) {
  if (typeof expression.path === 'function' || typeof expression !== 'object') {
    ...
    expression.id = this._splitPath(expression.id); 
  }
...
Model.prototype._initQueries = function(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (query.idsPath && query.idsPath.length > 0 && this._isLocal(query.idsPath[0])) {
      this._setNull(query.idsPath, ids.slice());
    }
...
function Query(model, collectionName, expression, source) {
  ...
  this.idsPath = expression && expression.id;
...
Query.prototype.sourceQuery = function() {
  ...
  else if (this.idsPath) {
    var ids = pathIds(this.model, this.idsPath);
    var expression = util.deepCopy(this.expression);
    delete expression.id;
    expression['_id'] = { $in: ids };
    return expression;
  }
