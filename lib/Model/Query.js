 ...
 * @param {Object} items[*][1] sourceQuery
  ...
  if (this.isPathQuery) {
    ...
    model.shareConnection.createFetchQuery(
      this.collectionName, this.sourceQuery, options, function(err, results, extra) {
        ...
        finishFetch(ids, extra);
      }
    );
  }

  function finishFetch(ids, extra) {
    ...
    if (extra !== void 0) {
      model._setDiff(query.extraSegments, extra);
    }
...
Query.prototype.subscribe = function(cb) {
  ...
  if (! this.model.fetchOnly) {
    return this._shareSubscribe(options, cb);
  }

  var query = this;
  var model = this.model;
  return model.shareConnection.createFetchQuery(
    this.collectionName, this.sourceQuery, options, function(err, results, extra) {
      if (err) return cb(err);
      var ids = resultsIds(results);
      query._onChange(ids);
...
Query.prototype.get = function() {
  var resultset = this.model._get(this.segments);
  var ids = resultset.ids;
  var extra = resultset.extra;
  var extraExists = (extra !== void 0);
  if (! ids && extraExists) return;
