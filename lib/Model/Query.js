...
Query.prototype.fetch = function(cb) {
  ...
  this.shareQuery.fetch(function(err) {
...
Query.prototype.subscribe = function(cb) {
  ...
  if (this.model.fetchOnly) {
    ...
    this.shareQuery.fetch(function(err) {
      if (err) return cb(err);
      query._onChange(query.shareQuery.results);
      cb();
    });
  } else {
    this.shareQuery.subscribe(function(err) {
      if (err) return cb(err);
      cb();
    });
  }
...
Query.prototype.unsubscribe = function(cb) {
  ...
  if (this.model.fetchOnly) {
    ...
    this.shareQuery.unsubscribe(group());
...
Query.prototype._onChange = function(results) {
  // Don't subscribe to docs if this was the result of a fetch. Fetch also
  // emits change events, but fetch only has an effect when not subscribed
  ...
  for (var i = 0; i < diff.length; i++) {
    ...
    if (item instanceof arrayDiff.InsertDiff) {
      ...
      var ids = this.model._remove(this.segments, item.index, item.howMany);
