...
function addListener(model, type, fn) {
  ...
  function refListener(segments, eventArgs) {
    ...
    var refs = node.getDescendantListeners([]);
    for (var i = 0; i < refs.length; i++) {
      var ref = refs[i];
...
Model.prototype.ref = function() {
  ...
  this.root._refs.remove(ref.fromSegments);
  this.root._refLists.remove(ref.fromSegments);
...
Model.prototype._removeAllRefs = function(segments) {
  var refs = this.root._refs.fromMap.getAllListeners(segments);
  for (var i = 0; i < refs.length; i++) {
    var ref = refs[i];
    this.root._refs.remove(ref.fromSegments);
    this._del(ref.fromSegments);
  }
  var refLists = this.root._refLists.fromMap.getAllListeners(segments);
...
Model.prototype._dereference = function(segments, forArrayMutator, ignore) {
  ...
  do {
    ...
    for (var i = 0, len = segments.length; i < len; i++) {
      ...
      refsNode = refsNode && refsNode.children && refsNode.children[segment];
      ...
      refListsNode = refListsNode && refListsNode.children && refListsNode.children[segment];
