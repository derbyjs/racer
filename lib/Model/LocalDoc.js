...
LocalDoc.prototype.setNull = function(segments, value, cb) {
  function setNull(node, key) {
    var previous = node[key];
    if (previous != null) return previous;
    node[key] = value;
    return value;
  }
  return this._apply(segments, setNull, cb);
};

LocalDoc.prototype.increment = function(segments, byNum, cb) {
  var self = this;
  ...
  function increment(node, key) {
    return node[key] = (node[key] || 0) + byNum;
  }
...
LocalDoc.prototype.stringInsert = function(segments, index, value, cb) {
  ...
  function stringInsert(node, key) {
    ...
    if (typeof previous === 'string') {
      return node[key] = previous.slice(0, index) + value + previous.slice(index);
    }
...
LocalDoc.prototype.stringRemove = function(segments, index, howMany, cb) {
  ...
  function stringRemove(node, key) {
    ...
    if (typeof previous === 'string') {
      ...
      return node[key] = previous.slice(0, index) + previous.slice(index + howMany)
    }
    return node[key] = '';
  }
...
LocalDoc.prototype.push = function(segments, values, cb) {
  function push(arr) {
...
LocalDoc.prototype.unshift = function(segments, values, cb) {
...
LocalDoc.prototype._arrayApply = function(segments, fn, cb) {
  ...
  if (!Array.isArray(arr)) {
    var err = new TypeError(this._errorMessage(
      fn.name + ' on non-array', segments, arr
    ));
    return cb(err);
  }
  var out = fn(arr);
