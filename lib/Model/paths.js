...
/**
 * Create a list of model objects scoped to a each underlying path to a particular path.
 * Example:
 *     var users = model.at('users');
 *     user[0].set('username', 'brian');
 *
 *  @param {String} segment
 *  @return {Array} of scoped models
 *  @api public
 */
Model.prototype.ats = function(subpath) {
  var path = this.path(subpath);
  var scope = this.scope(path);
  var items = scope.get();
  var ats = [];

  for(var key in items) {
    ats.push(this.scope(scope.path(key)));
  }
