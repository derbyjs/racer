...
Model.INITS.push(function(model) {
  ...
  function fnListener(segments, eventArgs) {
    ...
    model.root._fns.inputListeners.forEach(segments, function(fn) {
      if (fn !== pass.$fn) fn.onInput(pass);
    });
    // Mutation affecting output path
    model.root._fns.outputListeners.forEach(segments, function(fn) {
      if (fn !== pass.$fn) fn.onOutput(pass);
    });
  }
});
...
function Fns(model) {
  ...
  this.inputListeners = new EventTree();
  this.outputListeners = new EventTree();
}
...
Fns.prototype.start = function(name, path, inputPaths, fns, options) {
  ...
  this.outputListeners.addListener(fn.fromSegments, fn);
  return fn._onInput();
};
...
Fns.prototype.stop = function(path) {
  ...
  if (fn) {
    ...
    this.outputListeners.removeListener(fn.fromSegments, fn);
  }
  return fn;
};
