...
Model.prototype.evaluate = function(name) {
  ...
  return this._fns.get(name, inputPaths, fns);
};
...
Model.prototype.start = function(name, subpath) {
  ...
  return this._fns.start(name, path, inputPaths, fns);
};

Model.prototype.stop = function(subpath) {
  var path = this.path(subpath);
  var fn = this._fns.stop(path);
  if (fn) this._del(fn.fromSegments);
};
...
Fns.prototype.get = function(name, inputPaths, fns) {
...
Fns.prototype.start = function(name, path, inputPaths, fns) {
  fns || (fns = this.nameMap[name]);
