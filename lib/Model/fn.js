...
function addFnListener(model) {
  ...
  model.on('all', function fnListener(segments, eventArgs) {
...
Fns.prototype.stopAll = function(segments) {
  var listeners = this.fromMap.getAllListeners(segments);
  for (var i = 0; i < listeners.length; i++) {
    this._removeInputListeners(listeners[i]);
  }
  this.fromMap.deleteAllListeners(segments);
};
