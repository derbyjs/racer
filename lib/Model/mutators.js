...
Model.prototype._defaultCallback = function() {
  if (this._defaultCallbackFn) return this._defaultCallbackFn;
  var model = this;
  return this._defaultCallbackFn = function defaultCallback(err) {
...
Model.prototype._mutate = function(subpath, fn, cb) {
  ...
  if (!collectionName || !id) {
    var message = fn.name + ' must be performed under a collection ' +
      'and document id. Invalid path: ' + path;
...
Model.prototype.add = function(subpath, value, cb) {
  ...
  if (!path || ~path.indexOf('.')) {
    var message = 'add must be performed on a collection. Invaid path: ' + path;
    var err = new Error(message);
    return cb(err);
  }
  ...
  value.id = id;
  var doc = this.getOrCreateDoc(path, id);
  var previous = doc.set([], value, cb);
  this.emit('change', [path, id], value, previous, true, this._pass);
  return id;
};
...
Model.prototype.setEach = function(object, cb) {
  var keys = Object.keys(object);
  for (var i = keys.length; i--;) {
    var key = keys[i];
    var value = object[key];
    // With ShareJS, all of these sets will be combined and sent as one
    // operation, and locally each of the sets will happen synchronously.
    // Thus, we can just put the callback on the last set.
    if (i) {
      this.set(key, value);
    } else {
...
Model.prototype.insert = function(subpath, index, values, cb) {
  ...
  function insert(doc, segments, fnCb) {
    doc.insert(segments.slice(2), index, values, fnCb);
...
Model.prototype.move = function(subpath, from, to, howMany, cb) {
  ...
  function move(doc, segments, fnCb) {
    ...
    model.emit('move', from, to, moved.length, true, model._pass);
    return moved;
  }
