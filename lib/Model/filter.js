  ...
  function filterListener(segments, eventArgs) {
    ...
    for (var path in map) {
      ...
      if (util.mayImpact(filter.inputSegments, segments)) {
        if(eventArgs[0] === 'remove' || eventArgs[0] === 'insert') { // Check if item is filtered out, then trigger a change on the refList but NOT on the referenced path (from)
          var ctx = filter.ctx;
          var items = filter.model._get(filter.inputSegments);
          var i = eventArgs[1];
          var item = eventArgs[2];
          if(!filter.filterFn.call(filter.model, item, i, items.slice(0).splice(i, 0, item), ctx)) {
            filter.update(pass, {silent: true});
            continue;
          }
        }
        filter.update(pass);
