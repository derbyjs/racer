...
Model.INITS.push(function(model) {
  ...
  function filterListener(segments, eventArgs) {
    ...
    // Adjusts indices in after according to removed silent/filtered indices
    function removePatch(after) {
      var i;
      var length;

      for(i = 0, length = after.length; i < length; i++) {
        if(index <= after[i]) after[i] += howMany;
      }
      return after;
    }

    // Adjusts indices in before according to inserted silent/filtered indices
    function insertPatch(before) {
      var i;
      var length;
      for(i = 0, length = before.length; i < length; i++) {
        if(index <= before[i]) before[i] += howMany;
      }
      return before;
    }
    ...
    for (var path in map) {
      ...
      if (util.mayImpact(filter.inputSegments, segments)) {
        if(eventArgs[0] === 'remove' || eventArgs[0] === 'insert') { // Send in with patch to avoid issues with filtered items, as well as unnecessary view updates
          var index = eventArgs[1];
          var howMany = (Array.isArray(eventArgs[2]) ? eventArgs[2].length : 1);

          filter.update(pass, {patch: {fn: (eventArgs[0] === 'remove' ? removePatch : insertPatch), after: (eventArgs[0] === 'remove')}});
          continue;
        }
        filter.update(pass);
      }
...
Filter.prototype.update = function(pass, options) {
  var ids = this.ids();
  // Due to issues with unecessary reloads due to that Racer returns an array of string, albeit the ids are actually numbers
  // If this issue is fixed in the getters functionality instead, this can be removed
  var options = options || {};
  options.equal = options.equal || function (a, b) {
    ...
    for(i = 0; i < lengthA; i++) {
      if(a[i] !== b[i].toString()) return false;
    }

    return true;
  };
  this.model.pass(pass, true)._setDiff(this.idsSegments, ids, options);
};
