...
function RemoteDoc(collectionName, id, data, model) {
  if (data) {
    var shareDoc = model._getOrCreateShareDoc(collectionName, id, data);
  } else {
    var shareDoc = model._getOrCreateShareDoc(collectionName, id);
  }
  ...
  shareDoc.on('del', function(isLocal, previous) {
    ...
    model.emit('change', [collectionName, id], [void 0, previous, isLocal, void 0]);
  });
  shareDoc.on('create', function(isLocal) {
    ...
    model.emit('change', [collectionName, id], [value, void 0, isLocal, void 0]);
  });
}
...
RemoteDoc.prototype.increment = function(segments, byNumber, cb) {
  ...
  var op = (previous == null) ?
    ...
    [new IncrementOp(segments, byNumber)];
  this.shareDoc.submitOp(op, cb);
...
RemoteDoc.prototype._onOp = function(op, isLocal) {
  ...
  if (defined(item.oi) || defined(item.od)) {
    ...
    model.emit('change', segments, [value, previous, isLocal, void 0]);
  ...
  // ListInsertOp
    ...
    var values = [item.li];
    model.emit('insert', segments.slice(0, -1), [index, values, isLocal, void 0]);

  // ListRemoveOp
  } else if (defined(item.ld)) {
    var index = segments[segments.length - 1];
    var removed = [item.ld];
    model.emit('remove', segments.slice(0, -1), [index, removed, isLocal, void 0]);
  ...
  // ListMoveOp
    ...
    var to = item.lm;
    var howMany = 1;
    model.emit('move', segments.slice(0, -1), [from, to, howMany, isLocal, void 0]);

  // StringInsertOp
  } else if (defined(item.si)) {
    var index = segments[segments.length - 1];
    var value = item.si;
    model.emit('stringInsert', segments.slice(0, -1), [index, value, isLocal, void 0]);

  // StringRemoveOp
  } else if (defined(item.sd)) {
    var index = segments[segments.length - 1];
    var howMany = item.sd.length;
    model.emit('stringRemove', segments.slice(0, -1), [index, howMany, isLocal, void 0]);
