...
RemoteDoc.prototype.set = function(segments, value, cb) {
  if (segments.length === 0 && !this.shareDoc.type) {
    if (value.id && typeof (console && console.trace) === 'function') {
      console.trace('Warning: Creating a document with an id property', value);
    }
    this.shareDoc.create('json0', util.copy(value), cb);
