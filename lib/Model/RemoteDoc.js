...
function RemoteDoc(collectionName, id, shareDoc, model) {
  if (!shareDoc) shareDoc = model.shareConnection.open(collectionName, id, 'json0');
  this.collectionName = collectionName;
  this.id = id;
  this.shareDoc = shareDoc;
  this.model = model;
  shareDoc.on('op', onOp.bind(this));
}
...
function onOp(op, isLocal) {
  ...
  // StringInsertOp
    ...
    var index = segments[segments.length - 1];
    var value = item.si;
    model.emit('stringInsert', segments, [index, value, isLocal]);

  // StringRemoveOp
  } else if (defined(item.sd)) {
    var index = segments[segments.length - 1];
    var howMany = item.sd.length;
    model.emit('stringRemove', segments, [index, howMany, isLocal]);
