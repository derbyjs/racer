...
function applyArrayDiff(model, doc, segments, diff, group, silent, before) {
  ...
  for (var i = 0, len = diff.length; i < len; i++) {
    ...
    if (item instanceof arrayDiff.InsertDiff) {
      ...
      // Remove
      if(silent !== false) var removed = doc.remove(docSegments, item.index, item.howMany, group());
      ...
      // Move
      if(silent !== false) var moved = doc.move(docSegments, item.from, item.to, item.howMany, group());
      if(silent !== true) model.emit('move', segments, [item.from, item.to, (moved || before.slice(item.index, item.index + item.howMany)).length, model._pass]);
    }
  }
}
