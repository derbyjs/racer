...
Model.prototype._setDiff = function(segments, value, options, cb, isLocal) {
  ...
  function setDiff(doc, docSegments, fnCb) {
    ...
    doDiff(model, doc, segments, before, value, equalFn, group, isLocal, isEach);
  }
  return this._mutate(segments, setDiff, cb);
};

function doDiff(model, doc, segments, before, after, equalFn, group, isLocal, isEach) {
  if (typeof before !== 'object' || !before ||
    ...
    model.emit('change', segments, [after, previous, isLocal, model._pass]);
    return;
  }
  if (Array.isArray(before) && Array.isArray(after)) {
    ...
    if (
      ...
      doDiff(model, doc, itemSegments, before[index], after[index], equalFn, group, isLocal);
      return;
    }
    var docSegments = segments.slice(2);
  ...
  if (!isEach) {
    ...
    for (var key in before) {
      ...
      model.emit('change', itemSegments, [void 0, previous, isLocal, model._pass]);
    }
  }
  ...
  for (var key in after) {
    ...
    var itemSegments = segments.concat(key);
    doDiff(model, doc, itemSegments, before[key], after[key], equalFn, group, isLocal);
  }
}
