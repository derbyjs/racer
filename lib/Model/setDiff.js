...
Model.prototype._setDiff = function(segments, value, options, cb) {
  ...
  function setDiff(doc, docSegments, fnCb) {
    ...
    doDiff(model, doc, segments, before, value, equalFn, group, isEach, silent);
  }
 ...
 */
function doDiff(model, doc, segments, before, after, equalFn, group, isEach, silent) {
  ...
  if (Array.isArray(before) && Array.isArray(after)) {
    ...
    applyArrayDiff(model, doc, segments, diff, group, silent);
    return;
  }
  if (!isEach) {
...
function applyArrayDiff(model, doc, segments, diff, group, silent) {
  var docSegments = segments.slice(2);
