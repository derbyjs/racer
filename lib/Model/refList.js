...
function patchFromEvent(type, segments, eventArgs, refList) {
  ...
  if (segmentsLength === fromLength) {
    ...
    if (type === 'remove') {
      ...
      model._remove(refList.idsSegments, index, howMany);
      return;
    }
  ...
  if (type === 'stringInsert') {
    ...
    model._stringInsert(refList.toSegments.concat(key), stringIndex, stringValue);
    updateIdForValue(model, refList, index, value);
    return;
  }
  if (type === 'stringRemove') {
    ...
    model._stringRemove(refList.toSegments.concat(key), stringIndex, howMany);
    updateIdForValue(model, refList, index, value);
    return;
  }
...
function setNewToValues(model, refList, values) {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    var key = refList.keyByItem(value);
    ids.push(id);
    model._setDiff(refList.toSegments.concat(key), value);
  }
  return ids;
}
function updateIdForValue(model, refList, index, value) {
  var id = refList.idByItem(value);
  model._setDiff(refList.idsSegments.concat(index), id);
}
...
function patchToEvent(type, segments, eventArgs, refList) {
    ...
    if (type === 'insert') {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        for (var j = 0; j < indices.length; j++) {
          model._setDiff(refList.fromSegments.concat(indices[j]), value);
        }
      }
      return;
    }
    ...
    if (type === 'remove') {
      ...
      for (var i = removeIndex, len = removeIndex + howMany; i < len; i++) {
        ...
        for (var j = 0, indicesLen = indices.length; j < indicesLen; j++) {
          model._set(refList.fromSegments.concat(indices[j]), void 0);
        }
      }
      return;
    }
  ...
  if (type === 'change') {
    ...
    if (oldIndices && !equivalentArrays(oldIndices, newIndices)) {
      ...
      for (var i = 0; i < oldIndices.length; i++) {
        model._set(refList.fromSegments.concat(oldIndices[i]), void 0);
      }
    }
    if (newIndices) {
      for (var i = 0; i < newIndices.length; i++) {
        model._set(refList.fromSegments.concat(newIndices[i]), value);
      }
    }
    return;
  }
  ...
  if (type === 'stringInsert') {
    ...
    for (var i = 0; i < indices.length; i++) {
      model._stringInsert(refList.fromSegments(indices[i]), stringIndex, value);
    }
    return;
  }
  if (type === 'stringRemove') {
    ...
    var howMany = eventArgs[1];
    for (var i = 0; i < indices.length; i++) {
      model._stringRemove(refList.fromSegments(indices[i]), stringIndex, howMany);
    }
    return;
  }
  // Array mutations will have already been updated via an object
...
function patchIdsEvent(type, segments, eventArgs, refList) {
  ...
  if (model._get(itemSegments) !== item) model._set(itemSegments, item);
}
