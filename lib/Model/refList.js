...
function FullPathRefList(model, from, to, ids, options) {
  RefList.call(this, model, from, null, ids, options);
  this.to = to;
  this.toSegments = [];
  if (!Array.isArray(to)) to = [to];
  for (var i = 0; i < to.length; i++) {
    var path = to[i];
    if (path) this.toSegments.push(path.split('.'));
  }
}
...
FullPathRefList.prototype.onMutation = function(type, segments, eventArgs) {
  if (util.mayImpactAny(this.toSegments, segments)) {
    patchToEvent(type, segments, eventArgs, this);
  } else if (util.mayImpact(this.idsSegments, segments)) {
    patchIdsEvent(type, segments, eventArgs, this);
  } else if (util.mayImpact(this.fromSegments, segments)) {
