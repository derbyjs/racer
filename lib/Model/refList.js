...
function addListener(model, type) {
  ...
  function refListListener(segments, eventArgs) {
    ...
    for (var from in fromMap) {
      ...
      if (pass === refList) continue;
      if (util.mayImpact(refList.idsSegments, segments)) {
        patchIdsEvent(type, segments, eventArgs, refList);
      } else if (util.mayImpact(refList.toSegments, segments)) {
        patchToEvent(type, segments, eventArgs, refList);
      } else if (util.mayImpact(refList.fromSegments, segments)) {
        patchFromEvent(type, segments, eventArgs, refList);
      }
    }
  }
}
...
function patchFromEvent(type, segments, eventArgs, refList) {
  ...
  var model = refList.model;
  var isLocal = eventArgs[eventArgs.length - 2];
  ...
  if (segmentsLength === fromLength) {
    if (type === 'insert') {
      ...
      var ids = setNewToValues(model, refList, values, isLocal);
      model._insert(refList.idsSegments, index, ids, null, isLocal);
      return;
    }
    ...
    if (type === 'remove') {
      ...
      model._remove(refList.idsSegments, index, howMany, null, isLocal);
      return;
    }
    ...
    if (type === 'move') {
      ...
      model._move(refList.idsSegments, from, to, howMany, null, isLocal);
      return;
    }
  ...
  if (segmentsLength < fromLength) {
    model._setDiff(refList.fromSegments, refList.get(), null, null, isLocal);
    return;
  }
  ...
  if (segmentsLength > fromLength + 1) {
    var message = 'Mutation on descendant of refList `from` should have been dereferenced';
  ...
  // original object
  if (type === 'change') {
    model._set(refList.toSegments.concat(key), value, null, isLocal);
    updateIdForValue(model, refList, index, value, isLocal);
    return;
  }
  ...
  if (type === 'stringInsert') {
    ...
    var outSegments = refList.toSegments.concat(key);
    model._stringInsert(outSegments, stringIndex, stringValue, null, isLocal);
    updateIdForValue(model, refList, index, value, isLocal);
    return;
  }
  if (type === 'stringRemove') {
    ...
    var outSegments = refList.toSegments.concat(key);
    model._stringRemove(outSegments, stringIndex, howMany, null, isLocal);
    updateIdForValue(model, refList, index, value, isLocal);
    return;
  }
  if (type === 'insert' || type === 'remove' || type === 'move') {
    var message = 'Array mutation on child of refList `from` should have been dereferenced';
    model.emit('error', new Error(message));
    return;
  }
}
function setNewToValues(model, refList, values, isLocal) {
  var ids = [];
  for (var i = 0; i < values.length; i++) {
    ...
    var key = refList.keyByItem(value);
    var outSegments = refList.toSegments.concat(key);
    model._setDiff(outSegments, value, null, null, isLocal);
    ids.push(id);
  }
  return ids;
}
function updateIdForValue(model, refList, index, value, isLocal) {
  var id = refList.idByItem(value);
  var outSegments = refList.idsSegments.concat(index);
  model._setDiff(outSegments, id, null, null, isLocal);
}
...
function patchToEvent(type, segments, eventArgs, refList) {
    ...
    if (type === 'remove') {
      ...
      for (var i = removeIndex, len = removeIndex + howMany; i < len; i++) {
        ...
        for (var j = 0, indicesLen = indices.length; j < indicesLen; j++) {
          var outSegments = refList.fromSegments.concat(indices[j]);
          model._set(outSegments, void 0, null, isLocal);
        }
      }
      return;
    }
  ...
  if (type === 'change' || type === 'load' || type === 'unload') {
    ...
    if (newIndices) {
      for (var i = 0; i < newIndices.length; i++) {
        ...
        model._set(outSegments, value, null, isLocal);
      }
    }
    return;
  }
  ...
  if (type === 'stringInsert') {
    ...
    for (var i = 0; i < indices.length; i++) {
      ...
      model._stringInsert(outSegments, stringIndex, value, null, isLocal);
    }
    return;
  }
  if (type === 'stringRemove') {
...
function patchIdsEvent(type, segments, eventArgs, refList) {
  ...
  if (segmentsLength === idsLength) {
    if (type === 'insert') {
      ...
      model._insert(refList.fromSegments, index, values, null, isLocal);
      return;
    }
    ...
    if (type === 'remove') {
      var index = eventArgs[0];
      var howMany = eventArgs[1].length;
      model._remove(refList.fromSegments, index, howMany, null, isLocal);
      return;
    }
    ...
    if (type === 'move') {
      ...
      var to = eventArgs[1];
      var howMany = eventArgs[2];
      model._move(refList.fromSegments, from, to, howMany, null, isLocal);
      return;
    }
  }
  ...
  if (model._get(itemSegments) !== item) {
    model._set(itemSegments, item, null, isLocal);
  }
}

Model.prototype.refList = function(from, to, ids, options) {
  ...
  if (!(options && options.noSet)) {
    refList.model._set(refList.fromSegments, refList.get());
  }
...
Model.prototype.removeRefList = function(from) {
  var fromPath = this.path(from);
  return this._refLists.remove(fromPath);
};
...
RefList.prototype.indicesByKey = function(key) {
  var item = this.model._get(this.toSegments.concat(key));
  return this.indicesByItem(item);
};
RefList.prototype.itemById = function(id) {
  var key = id;
  return this.model._get(this.toSegments.concat(key));
};
RefList.prototype.itemByIndex = function(index) {
  var id = this.idByIndex(index);
  return this.itemById(id);
};
RefList.prototype.keyByIndex = function(index) {
  return this.idByIndex(index);
};
