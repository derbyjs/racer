...
Model.INITS.push(function(model) {
  ...
  addInternalListener(root);
});

Model.INTERNAL_EVENTS.$addRefList = true;
function addInternalListener(model) {
  model.on('$addRefList', function addRef(refList) {
    model._refLists.add(refList);
  });
}
...
Model.prototype.refList = function() {
  ...
  this._setArrayDiff(refList.fromSegments, refList.get());
  // We don't want the ref to get added until after the change event where its
  // set gets emitted. Thus, emit a special internal event, since events get
  // queued up and emitted serially
  this.emit('$addRefList', refList);
