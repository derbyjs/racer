...
RefList.prototype.indicesByItem = function(item) {
  ...
  // Ensure ids are strings; Ensure all ids are strings to avoid faulty comparison where some ids are stored as ints (e.g. on idsSegments) and some as strings (e.g. when grabbing from toSegments in idByItem);
  for(i = 0, length = ids.length; i < length; i++) ids[i] = ids[i].toString();
