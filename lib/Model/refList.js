...
function patchFromEvent(type, segments, eventArgs, refList) {
  ...
  if (segmentsLength === fromLength) {
    ...
    if (type === 'remove') {
      ...
      var values = model._remove(refList.idsSegments, index, howMany);
      // Delete the appropriate items underneath `to` if the `deleteRemoved`
      // option was set true
      if (refList.deleteRemoved) {
        for (var i = 0; i < values.length; i++) {
          var key = refList.keyByItem(values[i]);
          model._del(refList.toSegments.concat(key));
        }
      }
      return;
    }
  ...
  // original object
  if (type === 'change') {
    model._set(refList.toSegments.concat(key), value);
  ...
  if (type === 'stringInsert') {
    ...
    model._stringInsert(outSegments, stringIndex, stringValue);
    updateIdForValue(model, refList, index, value);
    return;
  }
...
RefList.prototype.dereference = function(segments, i) {
  ...
  remaining[0] = this.keyByIndex(remaining[0]);
  return this.toSegments.concat(remaining);
};

function FullPathRefList(model, from, to, ids, options) {
  RefList.call(this, model, from, to, ids, options);
}

FullPathRefList.prototype = new RefList;

FullPathRefList.prototype.get = function() {
  var ids = this.model._get(this.idsSegments);
  if (!ids) return [];
  var out = [];
  for (var i = 0; i < ids.length; i++) {
    var path = ids[i];
    out.push(this.model.get(path));
  }
  return out;
};
FullPathRefList.prototype.dereference = function(segments, i) {
  var path = this.idByIndex(segments[i + 1]);
  var remaining = segments.slice(i + 2);
  return path.split('.').concat(remaining);
};
// FullPathRefList.prototype.keyByItem = function(item) {
// };
// FullPathRefList.prototype.idByItem = function(item) {
// };
// FullPathRefList.prototype.indicesByItem = function(item) {
// };
// FullPathRefList.prototype.indicesByKey = function(key) {
// };
// FullPathRefList.prototype.itemById = function(id) {
// };
// FullPathRefList.prototype.itemByIndex = function(index) {
// };
// FullPathRefList.prototype.keyByIndex = function(index) {
// };
// FullPathRefList.prototype.idByIndex = function(index) {
// };
...
RefLists.prototype.add = function(from, to, ids, options) {
  var refList = (options && options.type === 'fullPath') ?
    new FullPathRefList(this.model, from, to, ids, options) :
    new RefList(this.model, from, to, ids, options);
