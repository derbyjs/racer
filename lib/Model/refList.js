...
function addListener(model, type) {
  ...
  function refListListener(segments, eventArgs) {
    ...
    for (var i = 0, len = segments.length; i < len; i++) {
      ...
      if (fromMap[subpath]) {
        patchFromEvent(model, type, segments, eventArgs, fromMap[subpath], i);
      }
      if (toMap[subpath]) {
        patchToEvent(model, type, segments, eventArgs, toMap[subpath], i);
      }
      if (idsMap[subpath]) {
        patchIdsEvent(model, type, segments, eventArgs, idsMap[subpath], i);
      }
    }
  }
}
...
function patchToEvent(model, type, segments, eventArgs, refList, segmentIndex) {
  ...
  if (segmentIndex === fromLength - 1) {
    if (type === 'insert') {
      ...
      var values = eventArgs[0];
      for (var i = 0, len = values.length; i < len; i++) {
        var index = refList.indexByKey(insertIndex + i);
        if (index === -1) continue;
        model.setDiff(refList.from + '.' + index, values[i]);
      }
      return;
    }
    ...
    if (type === 'remove') {
      ...
      for (var i = removeIndex, len = removeIndex + howMany; i < len; i++) {
        var index = refList.indexByKey(i);
        if (index === -1) continue;
        model.del(refList.from + '.' + index);
      }
      return;
    }
  ...
  var index = refList.indexByKey(segments[segmentIndex + 1]);
  if (index === -1) return;
...
Model.prototype.refList = function(from, to, ids) {
  ...
  to = this.path(to);
  ids = this.path(ids);
  this._refLists.add(from, to, ids);
};

Model.prototype.removeRefList = function(from) {
  from = this.path(from);
  this._refLists.remove(from);
};
...
RefList.prototype.get = function() {
  var out = [];
  var items = model.get(this.to);
  var ids = model.get(this.ids);
...
RefList.prototype.indexByKey = function(key) {
