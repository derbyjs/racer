...
function patchToEvent(type, segments, eventArgs, refList) {
  ...
  if (segmentsLength === toLength) {
    if (type === 'insert') {
      ...
      // If autoPatch, then auto patch instead of updating any content 
      // (or, it will be useless to look for any content to update since 
      // there will be no content to update since the new indices are new).
      if(refList.autoPatchIndices && Array.isArray(model._get(refList.toSegments))) {
        var indices = model._get(refList.idsSegments);
        var i;
        var length = indices.length;

        docSetWrapper = function(segments, value) {
          docSet = function(doc, docSegments, fnCb) {
            doc.set(docSegments, value, fnCb);
          };
          return model._mutate(segments, docSet);
        };

        for(i = 0; i < length; i++) {
          if(indices[i] >= insertIndex) docSetWrapper(refList.idsSegments.concat(i), indices[i] + howMany);
        }
        return;
      }
    ...
    if (type === 'remove') {
      ...
      if(refList.autoPatchIndices && Array.isArray(model._get(refList.toSegments))) {
        ...
        docRemoveWrapper = function(segments, index, howMany) {
          docRemove = function(doc, docSegments, fnCb) {
            var removed = doc.remove(docSegments, index, howMany, fnCb);
            model.scope('').emit('remove', segments, [index, removed, pass]);
          };
          return model._mutate(segments, docRemove);
        };

        for(i = indices.length - 1; i >= 0; i--) {
          // Index is within the removed, remove it (and emit remove event as well)
          if(indices[i] >= removeIndex && indices[i] < removeIndex + howMany) {
            docRemoveWrapper(refList.idsSegments, i, 1);
          }
          // Index is after the removed, adjust values
          if(indices[i] >= removeIndex + howMany) {
            docSetWrapper(refList.idsSegments.concat(i), indices[i] - howMany);
          }
        }
        return;
      }
...
function RefList(model, from, to, ids, options) {
  ...
  this.autoPatchIndices = options && options.autoPatchIndices;
}
...
RefList.prototype.idByItem = function(item) {
  ...
  for (var key in items) {
    if (item === items[key]) return Array.isArray(items) ? parseInt(key, 10) : key; // Always return integer if array
  }
};
