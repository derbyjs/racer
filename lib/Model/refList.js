    ...
    if (type === 'remove') {
      ...
      if (refList.deleteRemoved) {
        for (var i = 0; i < values.length; i++) {
          model._del(refList.toSegmentsByItem(values[i]));
        }
      }
      return;
    }
...
FullPathRefList.prototype.dereference = function(segments, i) {
  ...
  return path && path.split('.').concat(remaining);
};
FullPathRefList.prototype.toSegmentsByItem = function(item) {
  var id = this.idByItem(item);
  return id && id.split('.');
};
