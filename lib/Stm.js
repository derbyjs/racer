var COMMIT, LOCK, LOCK_CLOCK_MASK, LOCK_TIMEOUT, LOCK_TIMEOUT_MASK, MAX_RETRIES, RETRY_DELAY, Stm, UNLOCK, redis, txn;
...
Stm = module.exports = function() {
  ...
  this.commit = function(transaction, callback) {
    ...
    return lock(locksLen, locks, base, function(err, lockVal, ops) {
      ...
      return client.eval.apply(client, [COMMIT, locksLen].concat(__slice.call(locks), [lockVal], [JSON.stringify(transaction)], [function(err, ver) {
        if (err) {
          throw err;
        }
        if (ver === 0) {
