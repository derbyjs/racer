...
, events: {
    init: function (model) {
      ...
      model.on('beforeTxn', function (method, args) {
        ...
        var refEmitter = new EventEmitter();
        refEmitter.on('refList', function (node, pathToRef, rest, pointerList, dereffed, pathToPointerList) {
          ...
          if (!rest.length) {
            ...
            if (indexArgs = mutator.indexArgs) for (var k = 0, len = indexArgs.length; k < len; k++) {
              j = indexArgs[k]
              arg = args[j];
              if (!arg) continue;
              id = arg.id;
        ...
        if (method === 'set' && typeof args[1] === 'function') {
          treeLookup(data.world, path, {getRef: getRef, skipLast: true}, refEmitter);
        } else {
          treeLookup(data.world, path, {getRef: getRef}, refEmitter);
        }
      });
    }
