...
, proto: {
  ...
  , _createFn: function (path, inputs, fn, destroy) {
      var prevVal, currVal
        ...
        , reInputChildren = regExpPathsOrChildren(inputs)
        , reInputParents = inputs.map(function(input){
            return regExpPathOrParent(input);
          })
      ...
      var listener = function listenerFnMutator (mutator, _arguments) {
        ...
        if (reInputChildren.test(mutatorPath)) {
          currVal = updateVal();
        } else {
          var didParentMutate = reInputParents.some(function(re){
            return re.test(mutatorPath);
          });
          if (didParentMutate) {
            currVal = updateVal();
          }
