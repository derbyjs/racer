...
function RefListener(model, from, getter) {
  ...
  function listenerRefMutator (mutator, _arguments) {
    var path = _arguments[0][0]
      , matches = []
      , i, len
    for (i = 0, len = patternRegExps.length; i < len; i++) {
      if (patternRegExps[i].test(path)) matches.push(i);
    }
    if (!matches.length) return;

    // Lazy cleanup of listener
    if (model._getRef(from) !== getter) {
      model.removeListener('mutator', listenerRefMutator);
      return;
    }
    ...
    for (i = 0, len = matches.length; i < len; i++) {
      ...
      // Construct the next de-referenced path to emit on. pathGenerator may also
  ...
  model.on('mutator', listenerRefMutator);
}
