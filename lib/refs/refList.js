    ...
    if (!rest.length) {
      ...
      if (pointerList) {
        ...
        for (var k = 0, len = pointerList.length; k < len; k++) {
          ...
          var doc = getDoc(domain, id, to, pathToRef)
          if (doc) node.push(doc);
        }
      }
      ...
      if (meta.refEmitter) {
        meta.refEmitter.onRefListMember(node, pointerList, dereffedKey + '.' + index, dereffed, id, rest, Array.isArray(domain) ? index : null);
      }
      var pathRelToDomain = Array.isArray(domain) ? index : id;
      return {node: node, path: dereffed + '.' + pathRelToDomain};
    }
  };
}
