...
redisInfo = module.exports =
  ...
  onStart: (client, callback) ->
    client.llen 'starts', (err, startsLength) ->
      throw err if err
      client.get 'ver', (err, ver) ->
        throw err if err
        client.lpush startValue(startsLength, ver), callback
  
  # This function is intended to be called by the Store every time it connects
  # to the Redis server.
  starts: (client, callback) ->
    client.lrange 'starts', 0, -1, (err, starts) ->
      throw err if err
      if starts.length is 0
        console.warn '''Redis server does not have any record of being started by the
        Rally Redis loader. This should not occur in production, and may
        be the result of a flush of the database.'''
