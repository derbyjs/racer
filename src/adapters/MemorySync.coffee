...
MemorySync:: =
  ...
  insert: (path, index, args..., ver, data) ->
    ...
    index = +index
    unless 0 <= index <= len
      throw new Error "Out of bounds: insert('#{path}', #{index}, #{args.join(', ')}) on array of length #{len}"
  ...
  remove: (path, index, howMany, ver, data) ->
    ...
    unless (0 <= index) && (index + howMany <= len)
      throw new Error "Out of bounds: remove('#{path}', #{index}, #{howMany}) on array of length #{len}"
  ...
  move: (path, from, to, howMany, ver, data) ->
    ...
    unless (0 <= from) && (from + howMany <= len) && (0 <= to) && (to + howMany <= len)
      throw new Error "Out of bounds: move('#{path}', #{from}, #{to}, #{howMany}) on array of length #{len}"
